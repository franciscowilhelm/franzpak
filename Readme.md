...
- `bgjm_status()` - Query status of a specific job (`"running"`,
  `"finished"`, or `"error"`)
- `bgjm_unresolved()` - Non-blocking check: `TRUE` while a job is still
  running (mirrors `mirai::unresolved()`)
- `bgjm_collect()` - Retrieve a job’s result plus metadata (blocks until
  it resolves)
- `bgjm_result()` - Retrieve just the result object of a job (blocks
  until it resolves)
...
pak::pak("franciscowilhelm/franzpak")
```
